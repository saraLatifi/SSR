...
# SARA: it seems that this method is for SSRM
...
def load_data_valid(train_file, valid_portion=0.1, maxlen=19, sort_by_len=False, shuffle=True):  # SARA: added shuffle parameter to control the shuffling
    ...
    if maxlen:
        ...
        for x, y, u in list(zip(train_set[0], train_set[1], train_set[2])):
            if len(x) < maxlen:
                ...
                new_train_set_x.append(x[:maxlen])  # SARA: get the last 'maxlen' interactions as session items
                new_train_set_y.append(y)
                new_train_set_u.append(u)
        train_set = (new_train_set_x, new_train_set_y, new_train_set_u)
        del new_train_set_x, new_train_set_y, new_train_set_u
    ...
    sidx = numpy.arange(n_samples, dtype='int32')
    if shuffle:
        numpy.random.shuffle(sidx)  # SARA - interactions are ordered and it's a time-aware recommendation problem. Therefore, no shuffling is needed.
    n_train = int(numpy.round(n_samples * (1. - valid_portion)))
    valid_set_x = [train_set_x[s] for s in sidx[n_train:]]
    valid_set_y = [train_set_y[s] for s in sidx[n_train:]]
    valid_set_u = [train_set_u[s] for s in sidx[n_train:]]
