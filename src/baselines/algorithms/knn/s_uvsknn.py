...
# from src.baselines.algorithms.extensions.reminder import Reminder
import sys
sys.path.insert(0, '../../src')
from baselines.algorithms.extensions.reminder import Reminder


class SUVMContextKNN:
    '''
    ...
    extending_mode: string
        ...
        lastViewed: extend the current user's session with the his/her last viewed items
        score_based: higher score: if the items appeared in more previous sessions AND more recently
    ...
    def __init__(self, k, sample_size=1000, sampling='recent', similarity='cosine', weighting='div',
                 ...
                 remind=True, extending_mode='lastViewed',
        ...
        if self.hasReminders:
            if remind_strategy == 'hybrid':
                self.reminder = Reminder(remind_strategy=remind_strategy, remind_sessions_num=remind_sessions_num, weight_base=weight_base, weight_IRec=weight_IRec, weight_SSim=weight_SSim)
            else:  # basic reminders
                self.reminder = Reminder(remind_strategy=remind_strategy, remind_sessions_num=remind_sessions_num, reminders_num=reminders_num, remind_mode=remind_mode)
    ...
    def fit(self, data, items=None, stream_data_total=None):
        '''
        ...
        if not stream_data_total is None:
            train = stream_data_total  # to do the calculations for idf with the whole data, not only with the new data
