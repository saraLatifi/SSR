...
class SUSequentialRules:
    ...
    def fit(self, data, test=None, stream_data_total=None):
        ...
        for row in train.itertuples(index=False):
            ...
            if session_id != cur_session:
                ...
                for i in range(1, self.steps + 1 if len(last_items) >= self.steps else len(last_items) + 1):
                    ...
                    if not prev_item in self.all_rules:
                        self.all_rules[prev_item] = dict()


                    if not item_id in self.all_rules[prev_item]:
                        userSet = set()
                        self.all_rules[prev_item][item_id] = [0, userSet]

                    if not user_id in self.all_rules[prev_item][item_id][1]:  # in userSet
                        self.all_rules[prev_item][item_id][1].add(user_id)
                    ...
                    self.all_rules[prev_item][item_id][0] += weight
        ...
        rules = self.all_rules.copy()  # streaming recommendation
