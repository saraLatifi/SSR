...
# import src.baselines.evaluation.loader as dl
...
def run_file(conf):
    ...
    if conf['type'] == 'single':
        run_single(conf)
    elif conf['type'] == 'opt' or conf['type'] == 'stream_opt':
        run_opt(conf)
    elif conf['type'] == 'bayopt':
        run_bayopt(conf)
    elif conf['type'] == 'stream':
        run_stream(conf)
    else:
...
def run_stream(conf):
    ...
    print('run test stream')

    candidate = conf['data']['candidate']
    ...
    if 'type' in conf['data']:
        if conf['data']['type'] == 'pickle':  # pickle file (that is prepared by GAG)
            train, candidate_list = dl.load_data_stream_pickle(conf['data']['folder'], conf['data']['prefix'], candidate_num=candidate, train_eval=False)
        else:
            raise RuntimeError('Unknown data type: {}'.format(conf['data']['type']))

    # for i in range(1, len(candidate_list)+1):
    #     print('start run for candidate ', str(i))
    #     send_message('start run for candidate ' + str(i))
    #     run_single_candidate(conf, train, candidate_list, candidate=i)
    ...
    for k, a in algorithms.items():
        eval_algorithm_stream(train, candidate_list, k, a, evaluation, metrics, results, conf)

    # write_results_csv(results, conf, iteration=candidate)

def run_stream_opt_single(conf, iteration, globals):
    '''
    ...
    if 'type' in conf['data']:
        if conf['data']['type'] == 'pickle':  # pickle file
            if 'opts' in conf['data']:
                train, test = dl.load_data_stream_pickle(conf['data']['folder'], conf['data']['prefix'],
                                       train_eval=train_eval, **conf['data']['opts'])
            else:
                train, test = dl.load_data_stream_pickle(conf['data']['folder'], conf['data']['prefix'], train_eval=train_eval)
        else:
            raise RuntimeError('Unknown data type: {}'.format(conf['data']['type']))
    else:
        print('data type does not supported !')
    ...
    for k, a in algorithms.items():
        eval_algorithm_stream(train, test, k, a, evaluation, metrics, results, conf, iteration=iteration, out=False, train_eval=True)
    ...
    if 'type' in conf['data']:
        if conf['data']['type'] == 'pickle':  # pickle file
            if 'opts' in conf['data']:
                ...
                train, test = dl.load_data_session_pickle(conf['data']['folder'], conf['data']['prefix'],
                                                          slice_num=slice,
                                                          **conf['data']['opts'])
            else:
                train, test = dl.load_data_session_pickle(conf['data']['folder'], conf['data']['prefix'],
                                                          slice_num=slice)
        elif conf['data']['type'] == 'hdf':  # hdf5 file
            if 'opts' in conf['data']:
                # ( path, file, sessions_train=None, sessions_test=None, slice_num=None, train_eval=False )
...
def run_single(conf, slice=None):
    ...
    if 'type' in conf['data']:
        if conf['data']['type'] == 'pickle':  # pickle file
            if 'opts' in conf['data']:
                # ( path, file, sessions_train=None, sessions_test=None, slice_num=None, train_eval=False )
                train, test = dl.load_data_session_pickle(conf['data']['folder'], conf['data']['prefix'], slice_num=slice,
                                                       **conf['data']['opts'])
            else:
                train, test = dl.load_data_session_pickle(conf['data']['folder'], conf['data']['prefix'], slice_num=slice)
        elif conf['data']['type'] == 'hdf':  # hdf5 file
    ...
    for k, a in algorithms.items():
        eval_algorithm(train, test, k, a, evaluation, metrics, results, conf, iteration=slice, out=False)
...
def run_opt(conf):
    ...
    for i in range(start, iterations):
        ...
        if conf['type'] == 'opt':
            ...
            run_stream_opt_single(conf, i, globals)
...
def eval_algorithm(train, test, key, algorithm, eval, metrics, results, conf, slice=None, iteration=None, out=True):
    ...
    #     results[key] = eval.evaluate_sequences(algorithm, test, train, negative_sampling=conf['negative_sampling_mode'], use_pop_random=conf['sampling_conf']['pop_random'], negative_sample_size=conf['sampling_conf']['sample_size'])
    # else:
    #     results[key] = eval.evaluate_sequences(algorithm, test, train, negative_sampling=conf['negative_sampling_mode'])
    for m in metrics:
        if hasattr(m, 'start'):
            m.stop(algorithm)
...
def eval_algorithm_stream(train, candidate_list, key, algorithm, eval, metrics, results, conf, iteration=None, out=True, train_eval=False):
    '''
    Evaluate one single algorithm
        --------
        train : Dataframe
            Training data
        candidate_list: list of Dataframes
            list of test sets
        key: string
            The automatically created key string for the algorithm
        algorithm: algorithm object
        ...
        conf: dict
            Configuration dictionary
        train_eval: boolean
            True: optimization phase, False: test phase
    '''
    ts = time.time()
    print('fit ', key)
    # send_message( 'training algorithm ' + key )
    ...
    if hasattr(algorithm, 'init'):
        algorithm.init(train, candidate_list)
    ...
    test = candidate_list[0]
    results[key] = eval.evaluate_sessions(algorithm, metrics, test, train)
    if out:
        write_results_csv(results, conf, iteration=1)
        # write_results_csv({key: results[key]}, conf, iteration=1)

    if not train_eval:  # only while test (not optimization) phase, where we have online training
        prev_data = train
        for idx in range(1, len(candidate_list)):
            test = candidate_list[idx]
            test_prev = candidate_list[idx - 1]
            # prev_data includes training data and all previous candidate sets
            prev_data = pd.concat([prev_data, test_prev], ignore_index=True)
            # new_data and all stream_data so far (i.e., training data and all previous candidate sets)
            algorithm.online_learning(test_prev, prev_data)  # online_learning(new_data, total_data)
            results[key] = eval.evaluate_sessions(algorithm, metrics, test, prev_data)
            if out:
                write_results_csv(results, conf, iteration=idx+1)
                # write_results_csv({key: results[key]}, conf, iteration=idx+1)
...
def load_evaluation(module):
    ...
    '''
    # return importlib.import_module('src.baselines.evaluation.' + module)
    return importlib.import_module('baselines.evaluation.' + module)
